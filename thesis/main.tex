...
\midrule
Transport & TCP sockets with length-prefixed frames, experimental UDP datagrams with JSON fragmentation, and relay-assisted TCP forwarding. \\
...
\begin{itemize}
    ...
    \item relay-assisted physical forwarding through registered public relay nodes;
...
Before a DHT publication is accepted, the publisher must include a lightweight proof-of-work nonce. The implemented proof is intentionally simple and is computed over the physical DHT key, the canonical record JSON, and the nonce:

\begin{equation}
    proof = SHA512(dht\_key \parallel record\_json \parallel nonce)
\end{equation}

The publication is accepted only when the proof hash satisfies the configured number of leading zero bits:

\begin{equation}
    leading\_zero\_bits(proof) \geq dht\_publish\_pow\_difficulty\_bits
\end{equation}

This mechanism does not provide a complete anti-abuse system, but it introduces a measurable computational cost before records can be inserted or refreshed in the distributed tables. Responsible nodes can verify the proof cheaply before storing the record, while publishers must search for a valid nonce.
