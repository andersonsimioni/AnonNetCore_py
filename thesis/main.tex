...
This work proposes a modular architecture for decentralized peer-to-peer overlay networks designed to support secure and anonymous communication, data persistence, and decentralized applications. The proposed design separates the networking, persistence, and security concerns into modular components, allowing the system to support different connection strategies, storage models, and cryptographic schemes without requiring changes to the overall architecture.

To validate the feasibility of the proposed approach, an experimental proof-of-concept implementation was developed. The prototype validates the main overlay flows over TCP in local and Docker-based environments, while the broader architecture discusses NAT traversal mechanisms such as UDP hole punching, relay-assisted communication, and future transport extensions. Post-quantum cryptographic primitives are investigated for secure key establishment, digital signatures, and message protection. The validation also considers the architectural requirements needed to support persistent distributed data and application-level services on top of the proposed overlay.
...
Este trabalho propõe uma arquitetura modular para redes overlay peer-to-peer descentralizadas projetada para suportar comunicação segura e anônima, persistência de dados e aplicações descentralizadas. O projeto proposto separa os componentes de rede, persistência e segurança em módulos independentes, permitindo que o sistema suporte diferentes estratégias de conexão, modelos de armazenamento e esquemas criptográficos sem necessidade de modificar a arquitetura geral.

Para validar a viabilidade da abordagem proposta, foi desenvolvida uma implementação experimental de prova de conceito. O protótipo valida os principais fluxos da rede overlay sobre TCP em ambientes locais e baseados em Docker, enquanto a arquitetura mais ampla discute mecanismos de travessia de NAT, como UDP hole punching, comunicação assistida por relays e extensões futuras de transporte. Primitivas criptográficas pós-quânticas são investigadas para estabelecimento seguro de chaves, assinaturas digitais e proteção das mensagens. A validação também considera os requisitos arquiteturais necessários para suportar dados persistentes distribuídos e serviços de aplicação sobre a rede overlay proposta.
...
Several techniques have been proposed to overcome NAT-related limitations. One of the most widely adopted approaches is the use of Session Traversal Utilities for NAT (STUN), which allows a node to discover its public-facing IP address and port as seen by external servers [5]. However, STUN alone is insufficient in more restrictive NAT scenarios, such as symmetric NATs [5].

Traversal Using Relays around NAT (TURN) addresses this limitation by relaying traffic through an intermediate server when direct communication cannot be established [5]. Although effective, TURN introduces additional latency and creates a degree of centralization, which may be undesirable in fully decentralized systems [5].

Interactive Connectivity Establishment (ICE) combines multiple techniques, including STUN and TURN, to systematically discover the best possible communication path between peers [5]. ICE attempts direct connectivity first and falls back to relay-based communication when necessary, improving reliability in heterogeneous network environments [5].

Another widely used technique in P2P systems is UDP hole punching, which enables two peers behind NATs to establish direct communication by coordinating simultaneous outbound packets [5]. This approach is particularly effective in NAT types that preserve endpoint mappings and is commonly used in decentralized applications due to its low latency and reduced reliance on centralized infrastructure [5].

Recent advancements have also explored the use of modern transport protocols such as QUIC, which operates over UDP and integrates features such as connection migration and improved congestion control. These properties make QUIC a promising candidate for NAT traversal in dynamic network conditions, especially when combined with P2P overlay mechanisms [5].

Despite these techniques, NAT traversal remains a non-trivial challenge, particularly in large-scale decentralized networks with heterogeneous NAT behaviors [5]. In such scenarios, hybrid approaches combining direct communication, relay mechanisms, and overlay routing are often required to ensure connectivity among all peers [5]. These challenges directly motivate the development of more robust communication schemes in decentralized P2P systems [5].
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/prisma.png}
\caption{PRISMA}
\end{figure}
...
The proposed decentralized network layer must provide a set of core functionalities to enable secure, scalable, and anonymous peer-to-peer communication. The main functional requirements are described as follows:
...
This separation between physical and virtual identities enhances privacy, security, and flexibility, allowing the system to operate as a fully decentralized and anonymous overlay network [5], [9], [10].
...
\[
dht\_key = H(namespace \parallel logical\_key)
\]
...
\[
root\_key = H(namespace \parallel logical\_key)
\]

Each shard stores only a bounded portion of the full item list:

\[
shard\_key_i = H(namespace \parallel logical\_key \parallel i)
\]
...
\[
H(\mathrm{pk\_virtual\_node\_owner} \parallel \mathrm{title}) \mapsto
\left\{
\begin{aligned}
&\mathrm{pk\_virtual\_node\_owner}: \mathrm{str},\\
&\mathrm{title}: \mathrm{str},\\
&\mathrm{type}: \mathrm{str},\\
&\mathrm{last\_modified}: \mathrm{str},\\
&\mathrm{target\_ref}: \mathrm{str},\\
...
The object identified by \(target\_ref\) may contain the mutable state of the resource, such as version data, roles, permissions, predecessor references, or other application-level metadata. This keeps the DPT compact, while resource evolution is represented by the sequence of pointed state objects rather than by growth inside the pointer table itself.
...
where \(tag\) represents a normalized semantic label, and each returned \(resource\_id\) identifies a logical resource that has previously been associated with that tag.

The main purpose of the DTT is to separate semantic lookup from storage resolution. Instead of overloading the storage layer with human-readable names, profile identifiers, post descriptors, or other mutable application metadata, the architecture delegates this first discovery stage to the DTT. In this way, peers may perform searches based on semantics, while the lower layers remain focused on content-addressed retrieval and routing.

When a query contains multiple tags, the requester may combine the returned result sets using policies such as intersection, union, ranking, or partial matching, depending on the needs of the application layer. The DTT itself does not need to interpret the semantic meaning of the query beyond maintaining the distributed association between normalized tags and candidate resources.
...
\begin{aligned}
&\mathrm{pk\_physical\_node}: \mathrm{str},\\
&\mathrm{endpoints}: \mathrm{list}[\mathrm{dict}[\mathrm{str}, \mathrm{object}]],\\
&\mathrm{transport\_methods}: \mathrm{list}[\mathrm{str}],\\
&\mathrm{reachability\_class}: \mathrm{str},\\
&\mathrm{relay\_capable}: \mathrm{bool},\\
&\mathrm{hole\_punch\_capable}: \mathrm{bool},\\
&\mathrm{protocol\_version}: \mathrm{str},\\
&\mathrm{feature\_flags}: \mathrm{list}[\mathrm{str}],\\
&\mathrm{last\_validated\_at}: \mathrm{str},\\
&\mathrm{status}: \mathrm{str},\\
&\mathrm{signature}: \mathrm{str}
\end{aligned}
\right\}
\]

where \(H(pk_{physical\_node})\) identifies the physical node, \(endpoints[]\) contains known communication endpoints, \(transport\_methods[]\) indicates supported communication mechanisms, \(reachability\_class\) describes the observed connectivity condition of the node, \(relay\_capable\) and \(hole\_punch\_capable\) indicate forwarding and NAT traversal capabilities, \(protocol\_version\) and \(feature\_flags[]\) describe compatibility properties, \(last\_validated\_at\) records the most recent successful validation time, and \(status\) reflects the currently known operational condition.
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/layers.png}
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/conn_man.png}
...
The Routing Mechanism is responsible for enabling message delivery across the overlay without requiring global knowledge of the network topology [1], [2], [3], [8], [9]. In the proposed architecture, routing is designed to prioritize anonymity over path optimality, ensuring that intermediate peers are unable to infer the full origin-destination relation of a communication [6], [9], [10].

Routing begins with a lookup in the Distributed Routing Table (DRT), whose role is limited to indirect resolution of logical destinations [1], [2], [3], [8]. More specifically, the DRT maps the identifier of a virtual node, represented as $H(pk_{virtual\_node})$, to a set of candidate physical entry points capable of leading traffic toward that destination [5], [8], [9]. Thus, the DRT does not store full end-to-end routes, nor does it define the complete hop-by-hop forwarding process. Instead, it only provides the initial physical nodes through which a route may begin [5], [8], [9].

\begin{figure}[H]
\centering
\thesisfiguregraphic{img/drt.png}
\caption{DRT}
\end{figure}
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/hop_by_hop.png}
\caption{Route construction}
\end{figure}

In a TTL-based route construction strategy, once one of these entry points is selected,
the sender constructs a route by choosing an initial physical node, a final physical
node, a unique route identifier (\textit{path\_id}), a route expiration interval, and a
time budget represented as a TTL in milliseconds [6], [9], [10].

The sender then estimates the communication delay to the
first hop and forwards a route creation request containing the encrypted
virtual-destination information, the final physical node, the chosen \textit{path\_id}, and
the remaining TTL after subtracting the estimated cost of the current hop [6], [9].
In this process, the virtual-destination information remains protected along the path,
so that intermediate forwarding peers handle the \textit{path\_id} only as an opaque routing
reference and cannot associate it with the target virtual node. Only the final physical
node designated as the entry point is able to resolve the received route information and
determine which virtual node is associated with that path.

At each intermediate hop, the route is extended locally without revealing the complete route structure [9], [10]. Each node only knows the previous hop and the next hop to which the route must be forwarded [9], [10]. To further reduce linkability, each node remaps the received \textit{path\_id} into a new local identifier before forwarding the route creation request. As a result, the same route is not represented by a single stable identifier from origin to destination. Instead, the route is translated hop by hop through local tables, so that intermediate nodes cannot associate the original identifier with the final virtual destination [9], [10].

A particularly important property of this mechanism appears at the final stage of route establishment. The last intermediate node does not learn the virtual destination itself; it only learns that the route must be forwarded to the designated final physical node [9], [10]. Only this final physical node is capable of resolving the route toward the intended virtual node. Consequently, even the final intermediate relay cannot determine which virtual identity is actually being reached, which strengthens the anonymity properties of the architecture [9], [10].
...
The proposed routing strategy is identity-oriented, that is, it is primarily designed to deliver messages toward virtual identities represented by $H(pk_{virtual\_node})$ [5], [9]. Routing for files or other persistent data objects is treated separately through another distributed structure [8]. In the case of stored files, the corresponding storage DHT is expected to return the ephemeral virtual nodes responsible for storing that content, after which routing can proceed toward those virtual identities using the same overlay forwarding principles [8], [10].

To prevent indefinite circulation of traffic, the architecture currently relies on TTL-based expiration [7], [8], [10]. Each route creation and forwarding process is bounded by a finite time budget, and packets or route fragments that exceed this bound are discarded. At the current stage of the design, this TTL mechanism is the primary loop-limiting strategy. Additional message-cache mechanisms may be incorporated in the future, and local caches with temporal expiration are left open as an architectural extension, but they are not mandatory for the current proposal [7], [8].

The routing mechanism does not attempt to always select the fastest possible path. On the contrary, the preferred strategy is to make correlation harder by avoiding overly direct or predictable routes whenever anonymity would benefit from additional indirection [6], [9], [10]. In this sense, routing is intentionally biased toward privacy preservation. Nevertheless, the architecture allows future policies in which users may explicitly choose to favor faster routes when they are willing to accept a lower anonymity margin [6].
...
The DRT itself is distributed across the network, although peers may cache previously resolved routes or destination information locally in order to reduce repeated lookup overhead [1], [2], [3], [8]. DRT records may include metadata such as destination identifier, candidate entry points, validity interval, route expiration, latency-related estimates, and auxiliary policy fields. These records are also signed in order to reduce the risk of poisoning or falsification [5], [9]. In the current design, publication of DRT entries is restricted to authorized entry points, with authorization being derived from signatures issued by the corresponding virtual node to the designated final physical node [9].

Overall, the Routing Mechanism combines indirect destination resolution through the DRT with local hop-by-hop forwarding based on remapped route identifiers [5], [8], [9], [10]. This allows the overlay to deliver messages without global topology awareness, without explicit exposure of virtual-to-physical bindings, and without resorting to classical flooding [7], [8], [9]. As a result, the routing model remains compatible with the modular, decentralized, and anonymity-oriented goals of the proposed architecture [5], [8], [9], [10].
...
where the route lifecycle defines the abstract process of creating, validating, using,
and expiring a route, while the route strategy defines the path-selection policy applied
during construction.
...
Therefore, rather than defining routing as a fixed algorithm, the proposed architecture
treats routing as a family of possible strategies that share the same conceptual route
layer. This allows the overlay to evolve without requiring the entire routing model to
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/peer_discovery.png}
...
In the current validation scenario, the module is instantiated with ML-DSA, derived from the CRYSTALS-Dilithium signature scheme, for digital signatures; ML-KEM, standardized from CRYSTALS-Kyber, for dynamic key exchange; AES-256-CBC for symmetric session encryption; and SHA-512 for hashing [13], [14], [26]. These primitives are adopted as a concrete proof-of-concept configuration rather than as immutable architectural constraints. The design remains intentionally open to future substitution of algorithms, although the present architecture assumes a fixed cryptographic suite shared by all participants [13], [14], [26].
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/ddt.png}
\caption{DDT}
\end{figure}
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/ephemeral_vns.png}
...
For data retrieval, lookup and routing remain logically distinct but operationally connected [8], [10]. The DDT is queried first to discover which ephemeral virtual nodes currently store the content identified by the requested hash [8], [10]. After that, the initiator performs DRT resolution for one of those storage virtual nodes in order to obtain an initial entry point and create an indirect route toward it [5], [8], [9], [10]. Thus, content retrieval is not performed directly from the DDT itself, but through a two-step procedure in which DDT provides storage holders and DRT enables anonymous communication with them [8], [10].
...
For direct message exchange between virtual nodes, the protocol treats the application payload as protected session data transmitted over the active route [9], [10]. Once a route and session are in place, the same secure context may be reused for multiple messages without requiring immediate route reconstruction or session renegotiation. This reuse reduces protocol overhead and allows sustained communication between the same participants while the route remains operational [8], [9].
...
After session establishment, all traffic exchanged between peers is encrypted using a symmetric session key [9], [13], [14]. This symmetric protection is the default rule for post-handshake communication and is responsible for providing confidentiality, integrity, and authenticity for hop-to-hop traffic [9], [13], [14]. Since the session key is derived from authenticated ephemeral key exchange material, session-level encryption also inherits authentication guarantees from the identity verification performed during the handshake [9], [13], [14].
...
For the current version of the architecture, the symmetric encryption mechanism is fixed rather than negotiated dynamically [13], [14]. This choice simplifies implementation and evaluation while still preserving the possibility of future modular evolution. Furthermore, the architecture is designed to support forward secrecy, so that the later compromise of a long-term identity key does not automatically expose previously established sessions [9], [13], [14].
...
\begin{aligned}
score(peer) ={}&
w_1 \cdot invalid\_signatures \\
&+ w_2 \cdot request\_rate \\
&+ w_3 \cdot failed\_lookups \\
&+ w_4 \cdot origin\_concentration \\
&+ w_5 \cdot malformed\_messages
\end{aligned}
\]
...
\[
record\_key = H(namespace \parallel logical\_key)
\]
...
\[
P = H(record\_key \parallel payload \parallel nonce)
\]
...
\(H(record\_key \parallel payload \parallel nonce)\) and checking whether the result satisfies the
...
\begin{figure}[H]
\centering
\thesisfiguregraphic{img/dtt.png}
\caption{DTT}
\end{figure}

In the proposed architecture, content discovery is performed through a lightweight two-stage resolution process that connects semantic discovery to current content availability [8], [10]. Rather than depending on a single distributed structure for both search and storage resolution, the system adopts two complementary distributed tables: the Distributed Tag Table (DTT) and the Distributed Data Table (DDT) [8].

The purpose of this organization is to preserve low lookup cost while still supporting semantic search and distributed storage [8]. In this model, the DTT is responsible for semantic filtering, whereas the DDT resolves candidate content objects into lightweight descriptors and currently available storage holders [8], [10].

First, the \textit{Distributed Tag Table} (DTT) acts as the semantic entry point for content search. It maps normalized tags directly to sets of candidate content hashes, allowing a peer to discover content objects through tag-based queries [8]. Conceptually, the structure may be represented as:

\[
H(tag) \rightarrow \{H(content_1), H(content_2), \dots\}
\]

When a query contains multiple tags, the requester may combine the returned sets through intersection or partial matching policies. As a result, the DTT provides an efficient and lightweight mechanism for semantic filtering without requiring full-text indexing or expensive query processing [8].

Second, the \textit{Distributed Data Table} (DDT) resolves each discovered content hash into a lightweight content descriptor together with the set of virtual storage nodes that currently advertise possession of that object [8], [10]. In the current design, the DDT therefore stores both the minimal metadata required to identify the object in search results and the references needed for later retrieval.

This allows the system to present human-readable search results, validate tag associations, and immediately obtain the currently available holders of the content through a single distributed lookup [8], [10].

From that point onward, the requester may use the normal routing mechanisms of the overlay to reach one of the available storage virtual nodes and retrieve the desired object [8], [10].

Overall, the complete discovery cycle can therefore be summarized as:

\[
DTT \rightarrow DDT
\]

That is, the DTT determines which content hashes are associated with the requested tags, and the DDT resolves those hashes into both lightweight content descriptors and the currently available storage holders [8], [10]. This decomposition keeps the discovery process computationally lightweight while preserving semantic search and distributed retrieval within the storage layer [8].
...
Overall, network rebalancing in the proposed architecture means keeping critical metadata and access paths as uniformly distributed as reasonably possible, while allowing ordinary content and most traffic patterns to remain decentralized, opportunistic, and lightweight [6], [8], [10]. This choice reflects the broader philosophy of the architecture: avoid single points of failure, preserve anonymity, and maintain acceptable operational continuity without imposing unnecessarily heavy coordination overhead [6], [8], [9], [10].
...
The lookup workflow may begin either from application-level semantics or from a known content hash. When semantic discovery is required, the requester first queries the Distributed Tag Table (DTT), which maps normalized tags or higher-level descriptors to candidate manifest identifiers. From the selected manifest, the requester obtains the corresponding content reference and proceeds with content-addressed resolution. If the hash of the desired object is already known, the process may start directly from \(H(content)\) [8], [10]. At the storage and network level, however, actual object retrieval remains content-addressed.

The lookup process may begin in two different ways, depending on whether the requester already knows the hash of the desired content object. If \(H(content)\) is already known, the requester may query the DDT directly [8], [10]. Otherwise, the requester may first perform an optional semantic lookup through the DTT, using normalized tags or other higher-level descriptors in order to obtain candidate manifest identifiers and, from them, the corresponding content reference.
...
\begin{enumerate}
    \item optionally query the DTT when the content hash is not yet known;
    \item obtain \(H(content)\);
...
From a cryptographic perspective, message routing operates with two protection layers. First, the message payload is protected in a session associated with the communicating virtual endpoints [9], [13], [14]. Second, each hop between physical nodes is also protected through its own secure session [5], [9], [13], [14]. Although these sessions differ in scope, they follow the same underlying establishment logic: a node generates an ephemeral key-exchange public key, signs it with its long-term signing identity, transmits it to the counterpart, derives a shared symmetric session key, and then uses that key to protect subsequent traffic [9], [13], [14]. As a result, the architecture simultaneously supports secure hop-by-hop forwarding and protected end-to-end communication between the relevant overlay endpoints [9], [10], [13], [14].
...
The implementation was intentionally optimized for clarity, protocol validation, and observability. As a result, some decisions were simplified when compared to a production network. For example, the prototype currently uses TCP transport, local or Docker-based clusters, SQLite persistence, and a simplified DHT routing model. More advanced features such as complete NAT traversal, QUIC transport, full Kademlia routing tables, formal anti-abuse mechanisms, and security auditing are treated as future work.
...
\midrule
Transport & TCP sockets with length-prefixed frames. \\
...
Symmetric encryption & AES-256-CBC for encrypted session payloads. \\
...
The MVP currently uses TCP as the only transport. UDP hole punching, QUIC, and relay mechanisms remain future implementation targets.
...
In the social PoC, the practical mapping is:
...
\begin{equation}
    key = SHA512(namespace \parallel \texttt{"|"} \parallel logical\_key)
\end{equation}
...
In this model, the protocol defines what must happen for a route to become valid: path construction, final physical node validation, virtual node authorization, route ping/pong validation, RTT measurement, and DRT publication. The strategy defines how the next hop is selected, which constraints must be respected, and which metric should be optimized during route construction.
...
\node[lifecycle, below=1.15cm of controlMid, fill=gray!20] (messages) {
    ...
    {\ttfamily ROUTE\_CREATE\_VALIDATE\_AND\_PUBLISH} $\rightarrow$ {\ttfamily ROUTE\_CREATE\_OK}
};
...
The MVP implements the route strategy \codebreak{random\_\allowbreak{}walk\_\allowbreak{}ttl}. The implemented route build message sequence is:
...
During route creation, each intermediate physical node stores only the local mapping required to forward packets. For example, an intermediate hop stores a mapping from a received \texttt{path\_id} to a generated \texttt{path\_id}. It does not need to know the full path or the final virtual identity. This design preserves the separation between physical routing and virtual semantics.
...
Local persistence is implemented with SQLite through SQLAlchemy. Persistent state includes identities, endpoints, DHT records, route metadata, content metadata, and operational logs. Runtime-only state such as active sockets, handshake state, cryptographic session secrets, and temporary buffers remains in memory.

The main persistence rule is that stable identifiers and metadata are persisted, while active cryptographic material and live network resources are not persisted without protection.
...
The social model is intentionally simple:
...
The DPT key used by the PoC is:

\begin{equation}
    logical\_key = \texttt{anonnet.social|}virtual\_node\_id
\end{equation}

The DPT record contains:

\begin{equation}
    target\_ref = content\_id
\end{equation}

Since the DPT is signed by the owner virtual node, other peers can verify that the pointer belongs to the claimed profile. To read a friend's profile, the application queries the friend's DPT, validates the pointer, resolves the referenced content in the DDT, downloads the content from a holder, and renders the posts in the feed.
...
The implementation includes scripts to run local cores, clusters, the social PoC, the Debug Console, and integration smokes. The following commands are representative examples:
...
\midrule
\codebreak{virtual\_\allowbreak{}session\_\allowbreak{}smoke.\allowbreak{}py} & Validates physical cluster setup, VN creation, route publication in DRT, virtual session establishment, and virtual keepalive. \\
\codebreak{virtual\_\allowbreak{}message\_\allowbreak{}smoke.\allowbreak{}py} & Validates active virtual sessions, message delivery through \codebreak{VIRTUAL\_\allowbreak{}SESSION\_\allowbreak{}DATA}, and handler response. \\
\codebreak{virtual\_\allowbreak{}content\_\allowbreak{}smoke.\allowbreak{}py} & Validates local content storage, DDT holder publication, virtual content metadata request, byte-range download, and DDT holder publication after download completion. \\
\codebreak{virtual\_\allowbreak{}api\_\allowbreak{}stress\_\allowbreak{}smoke.\allowbreak{}py} & Exercises the local API under repeated operations involving VN creation, DRT resolution, virtual sessions, and messages. \\
\codebreak{virtual\_\allowbreak{}api\_\allowbreak{}local\_\allowbreak{}vn\_\allowbreak{}stress\_\allowbreak{}smoke.\allowbreak{}py} & Validates communication between two VNs hosted in the same core through the normal DRT/DPNT/route path, without local shortcuts. \\
\codebreak{debug\_\allowbreak{}state\_\allowbreak{}smoke.\allowbreak{}py} & Validates operational sanity, including active nodes, unique peers, session limits, DHT duplication checks, and debug snapshots. \\
\codebreak{core\_\allowbreak{}full\_\allowbreak{}flow\_\allowbreak{}smoke.\allowbreak{}py} & Executes the main integration smokes in sequence, reusing cluster and core instances when possible. \\
\codebreak{social\_\allowbreak{}dom.\allowbreak{}js} & Validates front-end social handlers using a fake DOM. \\
\codebreak{run\_\allowbreak{}social\_\allowbreak{}smoke.\allowbreak{}py} & Validates the social PoC with a real core, including profile creation, DPT/DDT publication, friendship, remote profile loading, feed construction, and direct message flow. \\
\bottomrule
\end{tabularx}
\end{table}
...
Figures~\ref{fig:cluster-log}, \ref{fig:poc-social-profile}, \ref{fig:poc-social-messaging}, \ref{fig:debug-console-overview}, \ref{fig:debug-console-network}, \ref{fig:debug-console-dht}, \ref{fig:debug-console-routes}, \ref{fig:debug-console-runtime}, and \ref{fig:smokes-passing} present screenshots collected from the repository or experimental execution.

\begin{figure}[H]
\centering
\IfFileExists{assets/img/cluster-log.png}{
    \thesisfiguregraphic{assets/img/cluster-log.png}
}{
    \fbox{
        \parbox{0.9\textwidth}{
            \centering
            Placeholder: cluster execution log showing multiple physical nodes, bootstrap, peer discovery, and route/DHT activity.
        }
    }
}
\caption{Cluster execution log with multiple physical nodes running in the experimental environment.}
...
\begin{figure}[H]
\centering
\IfFileExists{assets/img/smokes-passing.png}{
    \thesisfiguregraphic{assets/img/smokes-passing.png}
}{
    \fbox{
        \parbox{0.9\textwidth}{
            \centering
            Placeholder: terminal output showing integration smokes and social PoC smokes passing.
        }
    }
}
\caption{Smoke tests validating the main protocol flows of the MVP.}
...
\begin{tabularx}{\textwidth}{L{0.23\textwidth} L{0.36\textwidth} Y}
\toprule
\textbf{Area} & \textbf{Current limitation} & \textbf{Future work} \\
\midrule
NAT traversal & The MVP uses TCP in local or Docker-based tests and does not implement complete NAT traversal. & Implement UDP hole punching, STUN/TURN support, relay fallback, and later QUIC-based transport. \\
...
Anti-abuse & There is no mature defense against Sybil attacks, flooding, spam, or coordinated malicious peers. & Add publication proof-of-work, peer scoring, rate limits per peer and namespace, invalid signature penalties, and origin concentration detection. \\
...
Reliability & Route execution is hop-by-hop and lacks advanced end-to-end reliability mechanisms. & Add retransmission, acknowledgments, congestion control, route failover, and session migration policies. \\
...
This chapter presented the implementation of the proposed architecture as a functional experimental MVP. The prototype implements a physical layer, virtual layer, DHT namespaces, route construction, route execution, virtual sessions, content transfer, local API, and a social proof of concept.

The implementation demonstrates that the proposed separation between physical nodes and virtual nodes is feasible, that distributed records can be used to locate physical nodes, virtual routes, content holders, and mutable pointers, and that an external application can use the network through a local API. The smoke tests and social PoC validate the main protocol flows expected from the architecture.

At the same time, the implementation remains intentionally limited. It is a research prototype designed to validate architecture and integration, not a hardened public network. Future work should focus on NAT traversal, complete Kademlia-style routing, stronger anti-abuse mechanisms, scalability tests, cryptographic hardening, formal auditing, and more realistic application-level privacy controls.
...
The experimental validation was performed through smoke tests, integration scripts, local clusters, debug snapshots, and the social proof of concept. These tests exercised the main protocol flows of the MVP, including bootstrap, peer validation, DHT publication and query, route maintenance, virtual session establishment, virtual message delivery, content transfer by byte ranges, API usage, and social profile synchronization. Therefore, although the prototype was evaluated in a controlled environment, it successfully demonstrated the functional viability of the proposed architecture.
...
Even with these limitations, the project contributes a concrete and extensible architecture for decentralized overlay networks. Its main contribution is not only the implementation of individual mechanisms, but the integration of physical nodes, virtual nodes, distributed tables, route construction, virtual sessions, content transfer, local APIs, and application-level proof of concept into a single coherent experimental system. This integration demonstrates that the proposed model can serve as a foundation for future research and development in secure decentralized applications.

Future work should focus on hardening the prototype and extending it toward real-world deployment conditions. The most relevant directions include implementing UDP hole punching and relay fallback, adding QUIC support, replacing the simplified DHT behavior with a complete Kademlia-like routing layer, introducing publication proof-of-work and peer scoring, improving protection against Sybil and flooding attacks, executing larger-scale experiments with churn and packet loss, strengthening local key protection, separating media objects from social profile state, and performing a formal security review.
