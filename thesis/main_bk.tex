...
\usepackage[scaled]{helvet} % Helvetica-compatible captions required by SBC
...
\usepackage[font={bf,sf,footnotesize},labelsep=period]{caption}
...
\captionsetup[figure]{justification=justified,singlelinecheck=true,margin=0.8cm}
\captionsetup[table]{justification=justified,singlelinecheck=true,margin=0.8cm}
...
\begin{resumopt}
Sistemas descentralizados reduzem a dependência central, mas enfrentam desafios de descoberta, conectividade heterogênea, disponibilidade de dados e segurança. Este trabalho propõe uma rede overlay P2P modular para comunicação orientada à privacidade, persistência distribuída e aplicações descentralizadas, separando transporte, identidades virtuais, roteamento, armazenamento e criptografia. Um MVP em Python valida TCP, UDP fragmentado, relays, tabelas distribuídas, estabelecimento de chaves e assinaturas pós-quânticas e criptografia autenticada em clusters locais e Docker. A travessia completa de NAT permanece como trabalho futuro. Testes integrados e uma PoC social demonstram conectividade heterogênea, sessões virtuais baseadas em rotas, referências persistentes de conteúdo e acesso de aplicações externas por uma API local.
\end{resumopt}
...
Content discovery separates the identity of an object from the virtual nodes that currently provide it. The following figure introduces this resolution chain before the individual lookup and transfer steps are discussed.
...
Once discovery, route creation, and session establishment have completed, application data follows the layered forwarding process summarized below. This overview links the application-visible message to the virtual and physical envelopes that carry it through the network.
