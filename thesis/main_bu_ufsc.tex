...
\titulo{Uma Arquitetura Modular para Redes Overlay Peer-to-Peer Descentralizadas com Suporte a Comunicação Segura, Persistência de Dados e Aplicações Descentralizadas}
\autor{Anderson Simioni Assunção}
\local{Florianópolis}
\data{2026}
...
\renewcommand{\imprimircapa}{%
  \begin{capa}
    \center
    \fontsize{12}{1.5}
    \includegraphics[width=1.84cm]{brasao_UFSC_vertical_sigla.pdf}\par
...
\hypersetup{
  ...
  pdfkeywords={redes descentralizadas}{peer-to-peer}{overlay}{privacidade},
  colorlinks=true,
  linkcolor=black,
  citecolor=black,
  filecolor=black,
...
\begin{folhadeaprovacao}
  ...
  \begin{center}
    ...
    \vspace{1.5\onelineskip}
    \rule{6cm}{1pt}\par
    Prof. Wesley dos Reis Bezerra, Dr.\\
    Avaliador\\
...
\begin{resumo}
Este trabalho apresenta o projeto, a implementação e a validação experimental de uma arquitetura modular para redes overlay peer-to-peer descentralizadas. A proposta separa nós físicos, responsáveis pela conectividade e pelo transporte, de nós virtuais, responsáveis pelas identidades e interações lógicas das aplicações. O protótipo AnonNetCore implementa descoberta de pares, tabelas distribuídas, construção e execução de rotas, sessões físicas e virtuais, encaminhamento por relay, transporte TCP e UDP experimental, entrega confiável e transferência de conteúdo por faixas de bytes. A segurança combina identificadores SHA-512, assinaturas ML-DSA, estabelecimento de chaves ML-KEM e criptografia autenticada AES-256-GCM-SIV. A validação utiliza clusters Docker, testes integrados, uma API HTTP/WebSocket, ferramentas de observabilidade e uma prova de conceito de rede social. Os resultados demonstram a integração funcional entre descoberta, roteamento, persistência distribuída, comunicação virtual e aplicações externas. O trabalho permanece um MVP experimental: travessia completa de NAT, endurecimento contra ataques, escalabilidade em redes públicas e auditoria formal de segurança são trabalhos futuros.

\vspace{\onelineskip}
\noindent\textbf{Palavras-chave}: redes peer-to-peer; redes overlay; sistemas descentralizados; privacidade; tabelas hash distribuídas; criptografia pós-quântica.
\end{resumo}

\begin{resumo}[Abstract]
\begin{otherlanguage*}{english}
This work presents the design, implementation, and experimental validation of a modular architecture for decentralized peer-to-peer overlay networks. The proposal separates physical nodes, responsible for connectivity and transport, from virtual nodes, responsible for application identities and logical interactions. The AnonNetCore prototype implements peer discovery, distributed tables, route construction and execution, physical and virtual sessions, relay forwarding, TCP and experimental UDP transport, reliable delivery, and byte-range content transfer. Security combines SHA-512 identifiers, ML-DSA signatures, ML-KEM key establishment, and AES-256-GCM-SIV authenticated encryption. Validation uses Docker clusters, integrated smoke tests, an HTTP/WebSocket API, observability tools, and a social-network proof of concept. Results demonstrate functional integration among discovery, routing, distributed persistence, virtual communication, and external applications. The work remains an experimental MVP: complete NAT traversal, attack resistance, public-network scalability, and formal security auditing remain future work.

\vspace{\onelineskip}
\noindent\textbf{Keywords}: peer-to-peer networks; overlay networks; decentralized systems; privacy; distributed hash tables; post-quantum cryptography.
\end{otherlanguage*}
\end{resumo}
...
\begin{siglas}
  \item[AAD] Associated Authenticated Data
  \item[AES] Advanced Encryption Standard
  \item[API] Application Programming Interface
  \item[DDT] Distributed Data Table
  \item[DHT] Distributed Hash Table
  \item[DPNT] Distributed Physical Nodes Table
  \item[DPT] Distributed Pointer Table
  \item[DRT] Distributed Route Table
  \item[DTT] Distributed Tag Table
  \item[KEM] Key Encapsulation Mechanism
  \item[MVP] Minimum Viable Product
  \item[NAT] Network Address Translation
  \item[P2P] Peer-to-Peer
  \item[PN] Physical Node
...
Peer-to-peer (P2P) networks are distributed systems in which nodes, referred to as peers, communicate directly with each other without relying on centralized coordination \cite{augustine2022byzantine}. Unlike traditional client-server architectures, P2P systems decentralize control, enabling each node to act both as a client and as a server \cite{augustine2022byzantine}. This paradigm enhances system robustness, scalability, and fault tolerance \cite{augustine2022byzantine}.

A fundamental characteristic of P2P networks is their ability to self-organize. Nodes dynamically join and leave the network, a phenomenon known as \textit{churn}, requiring the system to continuously adapt its topology and routing structures \cite{augustine2022byzantine}. Efficient handling of churn is essential to maintain connectivity and ensure reliable data dissemination in large-scale environments \cite{augustine2022byzantine}.

P2P networks can be broadly classified into structured and unstructured overlays \cite{mehr2019unstructured}, \cite{augustine2022byzantine}. Unstructured P2P networks, such as early file-sharing systems, rely on flooding or random walks for resource discovery, which may lead to high communication overhead and limited scalability \cite{mehr2019unstructured}. In contrast, structured P2P networks employ well-defined topologies and deterministic routing mechanisms, typically based on Distributed Hash Tables (DHTs) \cite{augustine2022byzantine}.

Distributed Hash Tables provide a scalable and efficient method for resource lookup by mapping keys to nodes in a deterministic manner \cite{augustine2022byzantine}. Classical DHT-based systems, such as Chord \cite{stoica2001chord}, organize nodes in a logical ring structure and guarantee lookup operations in logarithmic time relative to the number of nodes. Similarly, Kademlia \cite{maymounkov2002kademlia} introduces a distance metric based on the XOR operation, enabling efficient routing and parallel query mechanisms. Other relevant systems include Pastry \cite{rowstron2001pastry} and Tapestry \cite{zhao2001tapestry}, which explore alternative routing strategies and proximity-aware optimizations.

These structured overlays significantly improve scalability and lookup efficiency; however, they also introduce challenges related to network dynamics, heterogeneity, and real-world deployment constraints \cite{augustine2022byzantine}. In particular, issues such as node churn, network latency, and the presence of nodes behind Network Address Translation (NAT) devices complicate direct peer-to-peer communication \cite{li2022unreachable}, \cite{augustine2022byzantine}.

Despite these limitations, P2P networks remain a fundamental building block for decentralized systems, enabling applications such as distributed storage, content distribution, and resilient communication infrastructures \cite{augustine2022byzantine}. Their inherent decentralization and scalability make them particularly suitable for scenarios requiring fault tolerance and autonomy, motivating their continued evolution and integration with modern networking and cryptographic techniques \cite{augustine2022byzantine}.

\section{NAT Traversal}

Network Address Translation (NAT) is widely used in modern networks to allow multiple devices to share a single public IP address. While NAT improves address utilization and provides a basic level of security by isolating internal networks, it introduces significant challenges for peer-to-peer (P2P) communication \cite{li2022unreachable}. In particular, nodes behind NATs are typically not directly reachable from external networks, which hinders the establishment of direct end-to-end connections \cite{li2022unreachable}.

Several techniques have been proposed to overcome NAT-related limitations. One of the most widely adopted approaches is the use of Session Traversal Utilities for NAT (STUN), which allows a node to discover its public-facing IP address and port as seen by external servers \cite{rfc8489}. However, STUN alone is insufficient in more restrictive NAT scenarios, since address discovery does not guarantee that an inbound path can be created for all NAT behaviors \cite{rfc8489}.

Traversal Using Relays around NAT (TURN) addresses this limitation by relaying traffic through an intermediate server when direct communication cannot be established \cite{rfc8656}. Although effective, TURN introduces additional latency and creates a degree of centralization, which may be undesirable in fully decentralized systems \cite{rfc8656}.

Interactive Connectivity Establishment (ICE) combines multiple techniques, including STUN and TURN, to systematically discover the best possible communication path between peers \cite{rfc8445}. ICE attempts direct connectivity first and falls back to relay-based communication when necessary, improving reliability in heterogeneous network environments \cite{rfc8445}.

Another widely used technique in P2P systems is UDP hole punching, which enables two peers behind NATs to establish direct communication by coordinating simultaneous outbound packets \cite{li2022unreachable}, \cite{rfc8489}, \cite{rfc8445}. This approach is particularly effective in NAT types that preserve endpoint mappings and is commonly used in decentralized applications due to its low latency and reduced reliance on centralized infrastructure \cite{li2022unreachable}, \cite{rfc8489}.

Recent advancements have also explored the use of modern transport protocols such as QUIC, which operates over UDP and integrates features such as connection migration and improved congestion control \cite{rfc9000}. These properties make QUIC a promising candidate for NAT traversal in dynamic network conditions, especially when combined with P2P overlay mechanisms \cite{rfc9000}.

Despite these techniques, NAT traversal remains a non-trivial challenge, particularly in large-scale decentralized networks with heterogeneous NAT behaviors \cite{rfc8489}, \cite{rfc8656}, \cite{rfc8445}. In such scenarios, hybrid approaches combining direct communication, relay mechanisms, and overlay routing are often required to ensure connectivity among all peers \cite{rfc8489}, \cite{rfc8656}, \cite{rfc8445}. These challenges directly motivate the development of more robust communication schemes in decentralized P2P systems \cite{li2022unreachable}, \cite{augustine2022byzantine}.

\section{Post-Quantum Cryptography}

The advancement of quantum computing poses a significant threat to widely used classical cryptographic systems. Algorithms such as RSA and Elliptic Curve Cryptography (ECC), which rely on the computational hardness of problems like integer factorization and discrete logarithms, can be efficiently broken using quantum algorithms such as Shor's algorithm \cite{shor1997quantum}. This scenario has motivated the development of post-quantum cryptography (PQC), which aims to design cryptographic schemes secure against both classical and quantum adversaries \cite{shor1997quantum}.

Among the various approaches to PQC, lattice-based cryptography has emerged as one of the most promising candidates due to its strong security guarantees and practical efficiency \cite{regev2009lwe}, \cite{bos2017kyber}. These schemes are based on hard mathematical problems such as the Learning With Errors (LWE) and Module-LWE problems, which remain computationally intractable even for quantum computers \cite{regev2009lwe}, \cite{bos2017kyber}.

A prominent example of a lattice-based cryptographic scheme is CRYSTALS-Kyber, which has been selected by the National Institute of Standards and Technology (NIST) as a standard for post-quantum key encapsulation mechanisms (KEMs) \cite{bos2017kyber}, \cite{nist2024fips203}. Kyber enables two parties to securely establish a shared symmetric key over an insecure channel, without requiring prior secret exchange \cite{bos2017kyber}, \cite{nist2024fips203}.

The operation of Kyber is divided into three main phases: key generation, encapsulation, and decapsulation \cite{bos2017kyber}, \cite{nist2024fips203}. In the key generation phase, a public and private key pair is created based on structured lattice operations. During encapsulation, a sender uses the recipient's public key to generate a ciphertext and derive a shared secret. In the decapsulation phase, the recipient uses their private key to recover the shared secret from the ciphertext \cite{bos2017kyber}, \cite{nist2024fips203}.

In practical systems, Kyber is often combined with symmetric cryptographic algorithms such as AES-256 to provide efficient and secure data encryption \cite{bos2017kyber}. In this hybrid approach, Kyber is used to securely exchange a symmetric key, which is then used to encrypt the actual data payload. In modern protocol designs, this symmetric protection should preferably use authenticated encryption modes, so that confidentiality and integrity are provided together rather than relying on encryption alone \cite{bos2017kyber}, \cite{rfc8452}. This design leverages the efficiency of symmetric cryptography while ensuring quantum-resistant key exchange \cite{bos2017kyber}.

The adoption of post-quantum cryptography is particularly relevant in decentralized and privacy-oriented systems, where long-term data confidentiality and resistance to future attacks are critical \cite{bos2017kyber}, \cite{nist2024fips203}. As a result, integrating PQC mechanisms into P2P architectures represents an important step toward building secure communication infrastructures resilient to emerging computational paradigms \cite{bos2017kyber}, \cite{nist2024fips203}.
...
In decentralized and peer-to-peer (P2P) systems, hash functions play a critical role beyond data integrity \cite{augustine2022byzantine}. They are commonly used for content addressing, message identification, and efficient data verification \cite{augustine2022byzantine}. In particular, hashing enables nodes to detect duplicate messages, avoid redundant transmissions, and mitigate network flooding \cite{mehr2019unstructured}, \cite{augustine2022byzantine}.

This property is especially relevant in gossip-based or broadcast-based communication models, where messages may propagate through multiple paths in the network \cite{augustine2022byzantine}. By maintaining a record of previously seen message hashes, nodes can prevent infinite propagation loops and significantly reduce unnecessary bandwidth consumption \cite{augustine2022byzantine}.

Additionally, cryptographic hash functions are often used in Distributed Hash Tables (DHTs), where keys are mapped to nodes in a deterministic manner \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{zhao2001tapestry}, \cite{augustine2022byzantine}. This enables efficient lookup and routing in large-scale decentralized systems \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{zhao2001tapestry}, \cite{augustine2022byzantine}.
...
\section{Distributed Hash Tables (DHTs)}
...
foundation for scalable distributed resolution mechanisms \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}.
...
nodes \cite{stoica2001chord}. Kademlia, on the other hand, introduces an XOR-based distance metric to
organize node identifiers and guide lookup operations \cite{maymounkov2002kademlia}.
...
\cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}. Rather than depending exclusively on flooding or unstructured
...
services in decentralized environments \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{augustine2022byzantine}.
...
\cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}.
...
\section{Kademlia and XOR-Based Routing}
...
\cite{maymounkov2002kademlia}. In this model, each node has a unique identifier,
...
\chapter{Systematic Literature Review Methodology}
...
\chapter{Selection Process (PRISMA)}
...
\chapter{Data Extraction and Analysis}
...
\toprule
\textbf{Work} & \textbf{Problem Addressed} & \textbf{Overlay Type} & \textbf{Routing Strategy} & \textbf{NAT Traversal} & {\footnotesize\textbf{Anonymity}} & {\footnotesize\textbf{Persistence}} & {\footnotesize\textbf{Scalability}} & \textbf{Post-Quantum Crypto} \\
\midrule

Spatial Path Selection~\cite{tosic2022spatial} &
...
A Fully-Distributed Scalable Peer-to-Peer Protocol for Byzantine-Resilient DHTs~\cite{augustine2022byzantine} &
...
Comparative Analysis of Unstructured P2P Networks~\cite{mehr2019unstructured} &
...
Unreachable Peers Communication Scheme~\cite{li2022unreachable} &
...
Garlic Cast~\cite{qian2016garlic} &
...
The proposed decentralized network layer was not directly derived from a single existing system. Instead, it was conceived as a hybrid architecture that integrates concepts and mechanisms inspired by multiple well-established paradigms in distributed systems and peer-to-peer networks \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

The design draws from onion routing approaches to provide layered communication and enhanced anonymity \cite{tosic2022spatial}, \cite{mirzaei2018pk}, \cite{qian2016garlic}, from unstructured peer-to-peer systems such as Gnutella to support flexible and resilient message propagation \cite{mehr2019unstructured}, \cite{augustine2022byzantine}, and from BitTorrent-like mechanisms to enable efficient data distribution and replication \cite{mehr2019unstructured}. Additionally, concepts related to depth-oriented search strategies and decentralized resource discovery were considered to improve routing adaptability and network exploration \cite{mehr2019unstructured}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.

These elements were combined into a unified model based on a fully decentralized peer-to-peer architecture, structured in logical layers and supported by distributed hash tables (DHTs) for indexing, routing, and data organization \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{zhao2001tapestry}, \cite{augustine2022byzantine}.

Rather than replicating any specific existing system, the proposed approach aims to synthesize the strengths of different paradigms while mitigating their individual limitations \cite{li2022unreachable}, \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. In particular, the architecture seeks to balance anonymity, scalability, data persistence, and connectivity in heterogeneous network environments, including scenarios involving NAT traversal \cite{li2022unreachable}, \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

This hybrid design reflects an intentional effort to move beyond isolated solutions and towards a more comprehensive and adaptable decentralized communication framework \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.
...
\midrule
P2P communication and discovery & Peers communicate without application servers and dynamically learn reachable participants despite churn \cite{augustine2022byzantine}, \cite{qian2016garlic}. \\
Heterogeneous connectivity & Direct transports, NAT traversal extensions, and relay-assisted communication support peers under different reachability conditions \cite{li2022unreachable}. \\
Decentralized routing & DHT-based resolution and hop-by-hop forwarding locate peers and deliver messages without a global network map \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}. \\
Secure exchange and identity & Cryptographic identities remain separate from physical addresses, and only authorized endpoints access protected content \cite{mirzaei2018pk}, \cite{qian2016garlic}, \cite{bos2017kyber}, \cite{nist2024fips203}. \\
Distributed persistence & Data and metadata are stored, retrieved, and replicated across peers for availability and fault tolerance \cite{mehr2019unstructured}, \cite{augustine2022byzantine}, \cite{qian2016garlic}. \\
Application support & Stable service abstractions expose messaging, lookup, storage, and retrieval to decentralized applications \cite{augustine2022byzantine}. \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
Scalability and efficiency & Growth should not require global state or flooding, and routing overhead and latency should remain bounded \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{tosic2022spatial}, \cite{mehr2019unstructured}, \cite{augustine2022byzantine}. \\
Churn and fault tolerance & Joins, departures, and failures should degrade local availability rather than invalidate the entire network \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{qian2016garlic}. \\
Low centralization & Bootstrap and relays may assist connectivity, but no component should control identities, routes, or application state \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. \\
Security and post-quantum readiness & Communication requires confidentiality, integrity, authenticity, and replaceable lattice-based primitives \cite{mirzaei2018pk}, \cite{shor1997quantum}, \cite{regev2009lwe}, \cite{bos2017kyber}, \cite{nist2024fips203}. \\
Privacy orientation & Logical identities and indirect routes should reduce direct association with physical endpoints without claiming formal global anonymity \cite{tosic2022spatial}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. \\
Extensibility and interoperability & Transport, routing, storage, and cryptographic modules should evolve independently while heterogeneous peers retain protocol compatibility \cite{li2022unreachable}, \cite{augustine2022byzantine}. \\
\bottomrule
\end{tabularx}
\end{table}
...
\midrule
Physical Node (PN) & Real process or device responsible for transports, physical sessions, discovery, NAT-related procedures, and relay forwarding \cite{li2022unreachable}, \cite{augustine2022byzantine}. \\
Virtual Node (VN) & Logical cryptographic identity hosted by a PN; one PN may host multiple VNs without publishing their local association \cite{mirzaei2018pk}, \cite{qian2016garlic}. \\
DHT and DHT Entry & Distributed coordination substrate and its signed records for physical discovery, routes, content holders, pointers, and tags \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}. \\
Protocol Message & Control-plane envelope used for discovery, lookup, session establishment, forwarding, storage, and maintenance \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}. \\
Data Object & Application bytes or metadata exchanged, referenced, replicated, or retrieved through the overlay \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. \\
\bottomrule
\end{tabularx}
\end{table}

Privacy orientation follows from this separation and hop-by-hop forwarding: physical information is exposed only where connectivity requires it, virtual identities represent application participation, and intermediate nodes require local forwarding state rather than a complete end-to-end route \cite{mirzaei2018pk}, \cite{qian2016garlic}.


\subsection{Resource Representation}

The overlay treats application resources as byte-oriented objects and assigns their semantics at the application layer \cite{augustine2022byzantine}. Immutable objects use content hashes, enabling content-addressed lookup and integrity verification; communication destinations use hashed VN identifiers \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. This uniform representation lets routing, discovery, and persistence operate without interpreting application data or exposing unnecessary physical context.

\subsection{Identity Model}

The dual-identity model decouples connectivity from logical participation \cite{li2022unreachable}, \cite{mirzaei2018pk}. A PN key pair authenticates transport-level actions, physical sessions, endpoints, and relay coordination; a separate VN key pair authenticates overlay identity, route authorization, and application communication \cite{mirzaei2018pk}, \cite{qian2016garlic}. DHTs use SHA-512 identifiers derived from public keys for compact lookup, while complete keys remain available when signature verification requires them \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. Encryption keys are session-specific: signed ephemeral KEM material establishes fresh symmetric secrets instead of statically reusing identity keys \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}.

\subsection{Overlay Abstraction}

The overlay exposes logical identifiers, distributed resolution, and hop-by-hop forwarding over any available physical transport \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. Applications therefore use stable messaging and persistence abstractions without managing addresses, relay selection, or transport-specific behavior; each forwarding node processes only the local routing information required for its next action \cite{mirzaei2018pk}, \cite{qian2016garlic}.


\subsection{Kademlia-Inspired DHT Responsibility Model}
...
\subsection{Core Distributed Structures}

\subsubsection{Distributed Pointer Table (DPT)}
...
\subsubsection{Distributed Tag Table (DTT)}
...
\subsubsection{Distributed Data Table (DDT)}
...
\subsubsection{Distributed Route Table (DRT)}
...
\subsubsection{Distributed Physical Nodes Table (DPNT)}
...
\subsection{Physical and Classical Network Layer}

This layer is responsible for the underlying classical network communication between
physical nodes \cite{li2022unreachable}, \cite{augustine2022byzantine}. It encompasses transport protocols such as TCP and UDP, as
well as the mechanisms required to establish connectivity between devices, including
NAT traversal and relay-assisted communication \cite{li2022unreachable}. In addition, this layer is also
...
identities \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.
...
routing, or application semantics \cite{augustine2022byzantine}. When the available transport mechanism is based
...
itself \cite{li2022unreachable}.

\subsection{Overlay Network Layer}

The overlay network layer is built on top of the classical communication layer and
provides decentralized interaction through logical identifiers \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. Its main
...
physical addressing \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}.
...
coordination among distributed entities \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}. This abstraction enables a consistent
interaction model across the network while decoupling logical communication from
the specific details of the underlying transport mechanisms \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}.

\subsection{Service Layer}

The service layer provides reusable functionalities on top of the overlay, such as distributed storage, messaging primitives, and data retrieval mechanisms \cite{augustine2022byzantine}, \cite{qian2016garlic}.

This layer exposes higher-level abstractions that simplify the development of applications, allowing them to use network capabilities without handling routing or communication details directly \cite{augustine2022byzantine}.

\subsection{Application Layer}

The application layer represents end-user systems built on top of the network, such as decentralized social platforms or communication tools \cite{augustine2022byzantine}.

Applications interact with the service layer to perform operations such as sending messages or storing data, without needing to manage lower-level networking or security mechanisms \cite{augustine2022byzantine}.


\section{System Components}
\subsection{Peer Management}

The Peer Management component is responsible for maintaining the local representation of entities participating in the overlay network \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. In the proposed architecture, participation is modeled through a strict separation between \textit{physical nodes} and \textit{virtual nodes}, so that logical identities in the overlay are decoupled from the underlying hosts that execute them \cite{li2022unreachable}, \cite{mirzaei2018pk}.

A physical node may host multiple virtual nodes simultaneously, establishing a $1:N$ relationship between execution environment and logical identities. However, each virtual node is associated with only one physical host at a given time. This design allows a physical device to act as an execution substrate for multiple overlay identities, while preventing the network from learning explicit ownership relations between them \cite{mirzaei2018pk}, \cite{qian2016garlic}.

The \textit{physical node public key} ($pk_{physical\_node}$) is not used as the main logical identity in the overlay. Instead, its role is limited to network access and reachability support \cite{li2022unreachable}, \cite{mirzaei2018pk}. In particular, physical node identifiers may appear in routing-related structures only as entry points through which a given virtual node may be reached \cite{li2022unreachable}. For example, the overlay may maintain associations of the form
\[
...
ownership relations \cite{li2022unreachable}, \cite{mirzaei2018pk}. In particular, such entry points do not necessarily correspond
...
The \textit{virtual node public key} ($pk_{virtual\_node}$) represents the primary logical identity of an entity in the overlay \cite{mirzaei2018pk}. To reduce storage and communication overhead, the network references virtual nodes through a derived identifier, denoted here as $H(pk_{virtual\_node})$, where $H(\cdot)$ is a cryptographic hash function \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. The complete public key does not need to be broadly exposed at the overlay level and may remain known only to parties that actually require it for cryptographic interaction \cite{mirzaei2018pk}, \cite{qian2016garlic}.

VNs may be persistent or expiring and may migrate between hosts without publishing a permanent PN association \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. Peer Management maintains controlled local identities plus limited remote metadata: identifiers, candidate entry points, transports, reachability, capabilities, freshness, and validation state. Physical peers are learned through bootstrap, exchange, observation, and DPNT fallback, then periodically validated and pruned \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.
...
Connection Management provides a common PN transport interface so overlay protocols remain independent of TCP, UDP, relay, or future connectivity mechanisms \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. Endpoint metadata belongs only to PNs:
\[
H(pk_{physical\_node}) \rightarrow \{endpoint_1, endpoint_2, \dots\},
\]
where each endpoint may represent a candidate communication method such as IPv4, IPv6, direct transport ports, relay endpoints, or parameters relevant to NAT traversal and hole punching \cite{li2022unreachable}.

Multiple candidates may coexist with reachability and validation state. Selection prefers lower-dependency methods while retaining fallback connectivity \cite{li2022unreachable}:
\begin{enumerate}
...
Selection considers advertised support, reachability, and observed conditions. Maintenance performs keepalives, endpoint validation, failure detection, and retries; another candidate is reconsidered when the active method fails. A relay may assist direct establishment or remain as the forwarding path when direct reachability is unavailable \cite{li2022unreachable}, \cite{augustine2022byzantine}.



\subsection{Routing Mechanism}

The Routing Mechanism is responsible for enabling message delivery across the overlay without requiring global knowledge of the network topology \cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. In the proposed architecture, routing is identity-oriented and privacy-oriented: messages are routed toward virtual identities, while intermediate physical nodes should not learn the complete origin-destination relation of a communication \cite{tosic2022spatial}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

Two operational phases must be distinguished. The first phase is \textit{route construction and publication}, in which a virtual node obtains one or more inbound route entries and publishes the corresponding reachability information in the Distributed Routing Table (DRT). The second phase is \textit{route resolution and execution}, in which another peer queries the DRT, selects one of the available entry points, and sends traffic through the already established route state. This distinction avoids treating the DRT as a table of complete end-to-end paths. The DRT stores candidate entry points and authorization metadata, while the actual forwarding state remains local to the physical nodes participating in each route \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}.

Routing resolution begins with a lookup in the DRT. More specifically, the DRT maps the identifier of a virtual node, represented as $H(pk_{virtual\_node})$, to a set of candidate physical entry points capable of receiving traffic toward that virtual destination \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}. Thus, the DRT does not store full hop sequences, nor does it prescribe the algorithm used to construct them. It only exposes the distributed information needed for a requester to start communication toward a reachable virtual identity \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}.
...
\]
where the listed physical nodes are entry points authorized to receive traffic destined to the corresponding virtual identity \cite{li2022unreachable}, \cite{mirzaei2018pk}. This preserves the indirect relation between virtual and physical entities, since the network never exposes an explicit and permanent binding between them \cite{mirzaei2018pk}, \cite{qian2016garlic}.

\thesisroutestrategyconstructiontikz

\thesishopbyhoptikz

Route construction proceeds hop by hop until a final PN accepts the entry-point role. The protocol defines lifecycle, authorization, validation, publication, and execution; the strategy selects each next hop and its termination rule. Figure~\ref{fig:ttl-route-strategy-example} shows \texttt{random\_walk\_ttl}, where each hop consumes a temporal budget, but other strategies may use hops, latency, bandwidth, reliability, diversity, or preselected onion-like paths \cite{tosic2022spatial}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

Each intermediate PN remaps the received \textit{path\_id} to local state and knows only adjacent forwarding actions. The final intermediate hop learns the final PN, not the destination VN; the final PN validates authorization and publishes the signed DRT entry \cite{mirzaei2018pk}, \cite{qian2016garlic}. Once active, \texttt{ROUTE\_DATA} forwarding is a local mapping operation independent of the construction strategy.

Content discovery first resolves providers through DTT/DPT/DDT and then routes toward a holder VN. TTL and \texttt{seen\_hash} are MVP safeguards, while future strategies may use other termination and deduplication controls. Route failure is detected by timeout and triggers another route or entry point rather than automatic mid-route repair \cite{li2022unreachable}, \cite{mehr2019unstructured}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.

\subsection{Route Strategy Abstraction}
\label{subsubsec:route-strategy-abstraction}

The route lifecycle is stable, while a replaceable strategy selects hops, termination conditions, and optimization goals for privacy, latency, bandwidth, stability, or diversity \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}:
...
Preselects hops and layered forwarding information, requiring greater prior peer knowledge \cite{tosic2022spatial}. \\
...
The Cryptographic Module protects physical and virtual sessions plus distributed records while preserving the PN/VN identity boundary \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{qian2016garlic}, \cite{bos2017kyber}, \cite{nist2024fips203}. Long-term ML-DSA keys authenticate identities, handshakes, and signed records; signed ephemeral ML-KEM material establishes fresh session secrets; AES-256-GCM-SIV protects payloads and selected public metadata through AAD; SHA-512 derives identifiers and content references \cite{bos2017kyber}, \cite{nist2024fips203}, \cite{nist2024fips204}, \cite{rfc8452}. The suite is fixed for interoperability in the MVP but remains replaceable at the architectural level.
...
The same authenticated handshake runs over direct or relayed transports. Physical sessions protect adjacent peers; virtual sessions protect communicating VNs end to end. Freshness fields, nonces, message identifiers, and duplicate controls provide replay resistance. Relays and storage peers process only the layer required by their role and do not receive virtual-session plaintext \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

\subsection{Storage Management}

Storage Management handles immutable, content-addressed application objects. A content change produces a new hash; DPT pointers represent mutable logical state without changing stored objects \cite{augustine2022byzantine}, \cite{qian2016garlic}.


\thesisddttikz


The DDT advertises expiring holder VNs rather than physical hosts or content bytes \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. Replication is interest-driven: consumers that retain an object may become providers, while uninterested peers incur no storage obligation \cite{mehr2019unstructured}, \cite{augustine2022byzantine}. Public objects remain readable; protected objects require application-managed keys even though transport is always session-protected \cite{qian2016garlic}, \cite{bos2017kyber}, \cite{nist2024fips203}.

\thesisephemeraltikz

Holder records are renewed or expire, keeping discovery aligned with active providers. Retrieved bytes must match their content hash. The network cannot guarantee global deletion after replication; an object disappears only when holders remove it and advertisements expire \cite{augustine2022byzantine}, \cite{qian2016garlic}. Location and access control remain separate: the DDT locates holders, while applications define who can interpret protected content.

\section{Communication Protocols}

Communication follows four transport-independent stages: discover and validate PNs; establish authenticated physical or virtual sessions; resolve a VN or content provider and obtain a route; exchange protected data \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. Fixed message semantics, explicit identifiers, bounded validity, signatures, and freshness controls support interoperability under churn and replay resistance. Control messages coordinate the network; data messages carry application payloads. Every participant receives only the metadata required by its local role.

\subsection{Peer Discovery Protocol}

Bootstrap returns public PN endpoints only. After an authenticated physical session is established, one-hop discovery exchanges bounded samples of validated peers and advertised transport, reachability, relay, and entry-point capabilities \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}. Candidates enter the local table only after liveness validation; background maintenance refreshes usable peers and removes repeated failures.

The DPNT stores renewable descriptors for already validated PNs and is queried only when local information is missing. It complements rather than replaces bootstrap and peer exchange, never carries VN information, and remains best-effort soft state rather than global membership \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.


\subsection{Connection Establishment}

Reachability alone is insufficient: a session becomes active only after authenticated establishment of a fresh symmetric key \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}. Transport selection prefers direct methods and falls back to assisted traversal or relay, but does not change handshake semantics.

The responder returns a signed ephemeral ML-KEM public key; the initiator verifies it, encapsulates a secret, signs the response, and sends the ciphertext; the responder decapsulates and confirms with encrypted control traffic. The procedure runs independently for adjacent PNs and communicating VNs, producing hop-by-hop and end-to-end keys. Bounded retries and keepalives prevent stale establishment state; persistent failure expires the session and requires a new attempt \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}.

\subsection{Routing and Lookup}

DRT lookup maps a destination VN identifier to authorized PN entry points; DDT lookup maps a content hash to holder VNs \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. Content retrieval therefore resolves a holder in the DDT and then resolves that VN in the DRT. Neither table returns a complete route.

Construction remaps \textit{path\_id} at every hop, so intermediates retain only incoming/outgoing local mappings and adjacent peers \cite{mirzaei2018pk}, \cite{qian2016garlic}. The MVP's TTL bounds construction, not later reuse. A failed active route is replaced through a new lookup rather than repaired locally. This separates distributed resolution from private local forwarding state and supports messaging and content without flooding.

\subsection{Data Exchange Protocol}

Data Exchange uses an active route and session; it does not discover peers, construct routes, or establish keys. The same encrypted virtual-session channel carries messages and byte-range content responses, while physical hops see only their protected forwarding layer \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}, \cite{bos2017kyber}, \cite{nist2024fips203}.
...
The security model treats a signing public key as a self-defining identity and separates persistent identity material from authenticated ephemeral sessions \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}.

\subsection{Identity and Authentication}

PN, persistent VN, and optional ephemeral VN identities use independent long-term signing keys \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{qian2016garlic}. Possession of the private key proves control without a certificate authority. Mutual session authentication binds signed ephemeral KEM material to that identity:

\[
pk_{sign} \rightarrow Sign(pk_{kex}) \rightarrow Session\ Key
\]

After signature validation and encrypted confirmation, the session key authenticates traffic within that session. PN and VN identities remain independent so connectivity-level presence is not published as application identity \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

\subsection{Encryption Mechanisms}

AES-256-GCM-SIV protects post-handshake traffic with confidentiality, integrity, and authentication \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}, \cite{rfc8452}. Fresh nonces are tracked per key. AAD covers visible fields that must remain stable, such as physical headers, route direction, virtual session identifiers, encryption flags, and control-message types; remapped hop-local \textit{path\_id} values are excluded from end-to-end AAD. Independent physical and virtual layers protect each hop and the final application payload, while optional object encryption protects stored private content.

\subsection{Post-Quantum Cryptography}

The model uses post-quantum public-key primitives and classical symmetric encryption: ML-DSA signs identities and control artifacts, ML-KEM establishes secrets, and AES protects bulk traffic \cite{shor1997quantum}, \cite{regev2009lwe}, \cite{bos2017kyber}, \cite{nist2024fips203}, \cite{nist2024fips204}, \cite{rfc8452}. It distinguishes:
...
Long-term keys are generated and stored locally and remain stable until intentionally replaced; session KEM pairs and symmetric keys are fresh and independent, including across concurrent sessions \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}. Revocation uses control messages and reannouncement in the MVP. Private content may use separate object keys. Without backup, loss of a signing private key means loss of that identity; recovery phrases and mature distributed revocation remain future work.

\subsection{Abuse Control and Sybil Resistance}

Open P2P systems cannot eliminate Sybil, Eclipse, flooding, or routing manipulation without authority or resource cost \cite{douceur2002sybil}. The architecture therefore combines complementary local mitigations rather than claiming perfect prevention. Optional identity admission may require a nonce satisfying $H(pk_{pn} \parallel nonce) < target$, making bulk identifier generation more expensive \cite{baumgart2007skademlia}.
...
Redundant lookup & Compare parallel or partially disjoint results to reduce Eclipse-style influence \cite{baumgart2007skademlia}, \cite{castro2002secure}. \\
Flood control & Apply rate limits, token buckets, bounded queues, and higher cost to writes, advertisements, and repeated lookups \cite{rfc2697}. \\
...
\subsection{Publication Proof-of-Work}
...
it, while critical DHT metadata is actively replicated \cite{augustine2022byzantine}, \cite{qian2016garlic}. Content may be
advertised through ephemeral virtual nodes, separating provider availability from a
long-term physical identity \cite{mirzaei2018pk}, \cite{qian2016garlic}.
...
holders can serve bytes without reading their plaintext \cite{augustine2022byzantine}, \cite{qian2016garlic}, \cite{bos2017kyber}, \cite{nist2024fips203}.
...
Retrievability remain a possible extension \cite{augustine2022byzantine}, \cite{qian2016garlic}.
...
fail \cite{augustine2022byzantine}, \cite{qian2016garlic}.

\subsection{Data Lifecycle and Expiration}
...
persistence, not absolute permanence \cite{augustine2022byzantine}, \cite{qian2016garlic}.

\subsection{Content Discovery and Resolution Model}
...
mutable application state \cite{augustine2022byzantine}, \cite{qian2016garlic}.
...
depends on voluntary holders \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.

\subsection{Node Churn Handling}
...
one becomes available before their liveness window expires \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

\subsection{Redundancy Strategies}
...
requiring global consensus or unlimited replication \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.
...
expired advertisements must be republished \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{qian2016garlic}.

\subsection{Network Rebalancing}
...
incremental, preserving service while ownership follows the evolving keyspace \cite{stoica2001chord},
\cite{maymounkov2002kademlia}, \cite{rowstron2001pastry}, \cite{augustine2022byzantine}.
...
exchange messages through the layered architecture \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

\subsection{Node Join Procedure}
...
virtual identity with published routes \cite{li2022unreachable}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}.

\subsection{Node Departure Procedure}
...
precise global tracking of online intervals \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.

\subsection{Data Lookup Process}
...
DPT, DDT, and DRT results may reduce repeated network queries \cite{augustine2022byzantine}, \cite{qian2016garlic}, \cite{bos2017kyber}, \cite{nist2024fips203}.

\subsection{Message Routing Process}
...
modified \cite{li2022unreachable}, \cite{mirzaei2018pk}, \cite{bos2017kyber}, \cite{nist2024fips203}.
...
properties \cite{tosic2022spatial}, \cite{augustine2022byzantine}, \cite{mirzaei2018pk}, \cite{qian2016garlic}.
...
\subsection{Relay-Assisted Physical Transport}
...
\subsection{DHT and Distributed Tables}
...
\section{Bootstrap and Peer Discovery}
...
\section{Route Construction and Route Execution}
...
\subsection{Hop-by-Hop Route Construction and Execution}
...
\section{Virtual Sessions and Virtual Messages}
...
\section{Reliable Session Payloads}
...
\begin{itemize}
    \item \raggedright \codebreak{PHYSICAL\_\allowbreak{}SESSION\_\allowbreak{}RELIABLE\_\allowbreak{}DATA} and \codebreak{PHYSICAL\_\allowbreak{}SESSION\_\allowbreak{}RELIABLE\_\allowbreak{}ACK} for hop-by-hop physical sessions;\par
...
\section{Social Proof of Concept}
...
\section{Validation and Test Results}
...
\section{Comparison with Related Systems and Concepts}
...
\bottomrule
\endlastfoot
Kademlia / DHTs~\cite{maymounkov2002kademlia} &
...
Peer-to-peer overlay networks~\cite{stoica2001chord}, \cite{maymounkov2002kademlia}, \cite{rowstron2001pastry} &
...
Tor / onion routing~\cite{dingledine2004tor} &
...
I2P / garlic routing~\cite{alothman2018i2p} &
...
Content-addressed systems~\cite{benet2014ipfs} &
...
Post-quantum cryptography~\cite{bos2017kyber}, \cite{nist2024fips203}, \cite{nist2024fips204} &
