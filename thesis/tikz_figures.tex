...
\node[appLayer] (app) {\textbf{Application Layer}\\[2pt]Decentralized Applications \quad Social Network \quad File Sharing \quad DApps};
...
\node[font=\bfseries\Large] at (0,5.25) {Strategy-Based Route Construction};
\node[text width=10.6cm, align=center, font=\small] at (0,4.40) {The route protocol keeps the lifecycle stable. The active route strategy chooses each next physical hop and defines when construction should finish.};
...
\node[block, fill=blue!6, draw=blue!55!black] (mapping) at (3.15,3.10) {Local mappings\\per hop};
\node[record] (drt) at (5.55,3.10) {DRT entry\\published};
...
\node[hop] (h1) at (-4.20,1.30) {PN};
\node[hop] (h2) at (-2.15,1.30) {PN};
\node[hop] (h3) at (-0.10,1.30) {PN};
\node[hop] (h4) at (1.95,1.30) {PN};
\node[finalnode] (final) at (4.00,1.30) {Final\\PN};

\draw[arrow] (h1.east) -- node[note, above=1pt] {path id map} (h2.west);
\draw[arrow] (h2.east) -- node[note, above=1pt] {path id map} (h3.west);
\draw[arrow] (h3.east) -- node[note, above=1pt] {path id map} (h4.west);
\draw[arrow] (h4.east) -- node[note, above=1pt] {path id map} (final.west);
...
\node[record, text width=2.55cm] (candidates) at (-3.70,-0.35) {Candidate peers\\and local state};
\node[record, text width=2.55cm] (objectives) at (-0.75,-0.35) {Optimization goal\\privacy, latency,\\bandwidth, reliability};
\node[record, text width=2.55cm] (termination) at (2.20,-0.35) {Termination rule\\strategy-specific};

\node[draw, rounded corners=5pt, fill=blue!4, text width=11.0cm, align=left, font=\small] at (0,-2.20) {$\bullet$ The protocol defines invariant route lifecycle messages and publication rules.\\$\bullet$ The strategy defines hop selection and stopping conditions.\\$\bullet$ Different strategies can optimize privacy, latency, bandwidth, reliability, or diversity without changing route execution.};
\end{tikzpicture}}
...
\node[font=\bfseries\Large] at (0,5.45) {Peer Discovery Process};
\node[text width=11.2cm, align=center, font=\small] at (0,4.65) {Public peers bootstrap the network, learn about private peers, and share reachability metadata for relay or hole-punch attempts.};

\node[stage] at (-5.35,3.78) {1. Bootstrap};
\node[seed] (seed1) at (-5.35,3.05) {DNS seeds};
\node[seed] (seed2) at (-5.35,2.32) {Hardcoded\\public nodes};

\node[stage] at (-2.40,3.78) {2. Public peer view};
\node[public] (pubA) at (-3.10,2.80) {P1};
\node[public] (pubB) at (-1.70,2.80) {P2};
\node[public] (pubC) at (-2.40,1.75) {P3};
\node[box, fill=blue!6, draw=blue!50!black, minimum width=2.75cm] (publicView) at (-2.40,0.55) {Known public\\peer exchange};
\draw[dashed, gray!65] (pubA) -- (pubB) -- (pubC) -- (pubA);

\node[stage] at (0.90,3.78) {3. Private peers};
\node[private] (privA) at (0.20,1.92) {N1};
\node[private] (privB) at (1.60,1.92) {N2};
\node[box, draw=red!55!black, fill=red!6, minimum width=3.05cm] (nat) at (0.90,0.72) {Behind NAT / Firewall};

\node[stage] at (4.45,3.78) {4. Connect};
\node[meta] (meta) at (4.45,2.62) {Published metadata\\reachable via relay\\or hole punching};
\node[relay] (relay) at (3.48,1.08) {Relay};
\node[box, fill=orange!7, draw=orange!70!black] (hole) at (5.42,1.08) {Hole punching\\attempt};
\node[public] (newPeer) at (4.45,-0.30) {P4};
...
\draw[dashedarrow] (nat.north west) -- (privA.south);
\draw[dashedarrow] (nat.north east) -- (privB.south);
\draw[arrow, shorten >=3pt, shorten <=3pt] (nat.west) -- (publicView.east);

\draw[arrow, shorten >=3pt, shorten <=3pt]
  (pubB.east) -- ++(0.42,0) |- node[pos=0.54, above, font=\tiny, fill=white, inner sep=1pt] {private reachability} (meta.west);
\draw[arrow] (meta.south west) -- (relay.north);
\draw[arrow] (meta.south east) -- (hole.north);
...
\node[font=\bfseries\Large] at (0,4.45) {Distributed Data Table (DDT)};
\node[text width=11.9cm, align=center, font=\small] at (0,3.82) {The DDT maps content hashes to virtual holders; it stores neither content bytes nor physical node identities.};
...
\draw[arrow] (table.east) -- node[font=\scriptsize, fill=white, inner sep=1pt, above, pos=0.62] {lookup} (a.west);
\draw[arrow] (table.east) -- (d.west);
...
\draw[dashedarrow] (c.east) -- (noPhysical.west);
\draw[dashedarrow] (f.east) -- (noPhysical.west);
...
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[
  block/.style={rectangle, rounded corners=5pt, minimum width=4.05cm, minimum height=3.75cm, text width=3.72cm, align=center},
...
\node[font=\bfseries\Large, blue!35!black] at (0,5.3) {Content Discovery and Resolution Model (Two-Stage)};
\node[text width=10.4cm, align=center, blue!35!black] at (0,4.55) {A lightweight process connecting semantic discovery to current content availability using two distributed tables.};
\node[side] (query) at (-6.35,1.0) {\textbf{Query}\\[5pt]Tags\\[3pt]\texttt{tag1}\\\texttt{tag2}\\\texttt{tag3}};
\node[greenBlock] (dtt) at (-3.0,0.85) {};
\node[font=\bfseries\scriptsize, text width=3.3cm, align=center] at (-3.0,2.42) {1. Distributed Tag Table\\(DTT)};
\node[align=center, font=\scriptsize] at (-3.0,1.58) {Semantic Entry Point};
\node[row] at (-3.0,0.92) {\texttt{tag1} $\rightarrow$ \{hA, hB, hC\}};
\node[row] at (-3.0,0.34) {\texttt{tag2} $\rightarrow$ \{hB, hD, hE\}};
\node[row] at (-3.0,-0.24) {\texttt{tag3} $\rightarrow$ \{hA, hF, hG\}};
\node[blueBlock] (ddt) at (1.82,0.85) {};
\node[font=\bfseries\scriptsize, text width=3.3cm, align=center] at (1.82,2.42) {2. Distributed Data Table\\(DDT)};
\node[align=center, font=\scriptsize, text width=3.35cm] at (1.82,1.58) {Resolution and Availability Layer};
\node[row] at (1.82,0.92) {hA $\rightarrow$ descriptor + vn1};
\node[row] at (1.82,0.34) {hB $\rightarrow$ descriptor + vn2};
\node[row] at (1.82,-0.24) {hC $\rightarrow$ descriptor + vn3};
...
\draw[arrow] (dtt.east) -- node[above, font=\tiny, fill=white, inner sep=1.5pt] {hashes} (ddt.west);
\draw[arrow] (ddt.east) -- node[above, font=\tiny, fill=white, inner sep=1.5pt] {holders} (overlay.west);
...
\node[draw=green!50!black, fill=green!8, rounded corners=4pt, text width=3.95cm, minimum height=1.15cm, align=left, font=\small] at (-4.45,-4.95) {\textbf{DTT}\\Maps tags to hashes.\\Semantic filtering.};
\node[draw=blue!50!black, fill=blue!8, rounded corners=4pt, text width=3.95cm, minimum height=1.15cm, align=left, font=\small] at (0,-4.95) {\textbf{DDT}\\Resolves hashes.\\Tracks holders.};
\node[draw=orange!70!black, fill=orange!8, rounded corners=4pt, text width=3.95cm, minimum height=1.15cm, align=left, font=\small] at (4.45,-4.95) {\textbf{Overlay Retrieval}\\Routes to storage VNs.\\Retrieves content.};
\end{tikzpicture}}
...
\resizebox{0.98\textwidth}{!}{%
\begin{tikzpicture}[
  panel/.style={rectangle, draw, rounded corners=4pt, fill=gray!8, minimum width=2.2cm, minimum height=1.25cm, text width=2.0cm, align=center, font=\scriptsize},
...
\foreach \i/\x/\t in {1/-5.0/DRT Query,2/-2.5/Entry Points,3/0/Hop-by-Hop Routing,4/2.5/Protected Communication,5/5.0/Reply Path} {
...
\node[ellipse, draw, minimum width=5.7cm, minimum height=2.95cm, fill=blue!5] (cloud) at (0.15,0.55) {};
\foreach \n/\x/\y/\label in {a/-1.65/0.15/A,b/-0.35/1.05/B,c/1.0/0.75/C,d/-0.15/-0.75/D,e/-1.35/-1.25/E,f/1.45/-1.15/F,g/0.45/-1.75/G,h/2.45/0.0/H} {
  \node[rectangle, draw, fill=gray!25, minimum width=0.45cm, minimum height=0.75cm] (n\n) at (\x,\y+0.55) {};
  \node[font=\scriptsize] at (\x,\y) {Node \label};
}
...
\node[draw, rounded corners=4pt, fill=blue!7, text width=2.9cm, minimum height=1.8cm, align=left, font=\small] at (-4.8,-3.85) {\textbf{Node knowledge}\\Previous hop and next hop only.};
\node[draw, rounded corners=4pt, fill=blue!7, text width=2.9cm, minimum height=1.8cm, align=left, font=\small] at (-1.6,-3.85) {\textbf{Protection}\\E2E layer plus hop layer.};
\node[draw, rounded corners=4pt, fill=blue!7, text width=2.9cm, minimum height=1.8cm, align=left, font=\small] at (1.6,-3.85) {\textbf{Failure}\\Retry entry point or route.};
\node[draw, rounded corners=4pt, fill=blue!7, text width=2.9cm, minimum height=1.8cm, align=left, font=\small] at (4.8,-3.85) {\textbf{Properties}\\Obfuscated path, no global route.};
\end{tikzpicture}}
...
\begin{tikzpicture}[
    ...
    note/.style={align=center, font=\scriptsize, text width=2.65cm}
]
...
\node[note, below=5mm of session] {Establishes keys and transports protected application messages.};
\end{tikzpicture}}
